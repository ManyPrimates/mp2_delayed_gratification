...
d_basel_sum <- d_basel%>%
  ...
  group_by(species, subject, delay,exchange_time)%>%
  summarise(n = n())%>%
  group_by(species, subject)%>%
  summarise(delay = max(delay),
         exchange = max(exchange_time))%>%
  mutate(exchange = as.numeric(exchange))
```
...
```{r}
d_basel_sum%>%
  pivot_longer(cols = c("delay", "exchange"), names_to = "type", values_to = "time")%>%
  ggplot(aes(x = subject, y = time, col = species, pch = type))+
...
```{r}
d_basel_sum%>%
  ggplot(aes(x = delay, y = exchange, col = species))+
  geom_jitter()+
  labs(x = "max delay", y = "actual exchange time")+
  geom_smooth(aes(x = delay, y = exchange),inherit.aes = F, method = "lm")+
  theme_minimal()
```
